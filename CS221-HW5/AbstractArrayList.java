...
public abstract class AbstractArrayList<T> implements ListADT<T> {
	...
	protected void expandCapacity() {
		T[] temp = (T[]) (Arrays.copyOf(list, list.length * 2));
		list = temp;
	}
	...
	public T removeLast() throws EmptyCollectionException {
		...
		rear--; // since we have the an empty slot at the end
		modCount++;
		T retVal = list[rear];
		list[rear] = null; // Avoiding Potential Memory leak
		return retVal;
	}
	...
	public String toString() {
		...
		if(!isEmpty()) strBuild.replace(strBuild.length()-2, strBuild.length(), "");
		strBuild.append("]");
	...
	private class ArrayListIterator implements Iterator<T> {
		int iteratorModCount; // the "version" of the list at the time the
